...
Here $n_d = T - (m-1)d$. `stat_op_bp(data, w; chart_choice, m, ljung_box=true)` sums the
summands over $d = 1, \ldots, w$, and `crit_val_op_bp(w; chart_choice, m)` returns the
...
odd and at even times are two independent `AR1(0.5, Normal(0, 1))` processes, which
...
summands of delay $d$ are functions of the pattern frequencies returned by `stat_op`. One
